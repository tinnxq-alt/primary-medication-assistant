...
- 门诊目录与补充资料按需并行加载：首屏直接显示已审计的 395 条数量，首次切换时再载入完整目录，不阻塞病房药库首屏；加载失败可直接重试，离线应用仍预缓存门诊资料。
- 门诊 395 个品规的适应症、用法用量、不良反应和注意事项已全部核验覆盖：129 个同药品名称（含剂型）、同成分规格品规安全复用现有资料，剩余 266 个品规按公开说明书逐条补齐并保留 HTTPS 来源。
...
每个 Pull Request 都会自动执行药品资料与代码质控。检查范围包括病房 164 个目录品规与核验库的一一对应、3 个品种永久删除及其余历史 ID 稳定性、门诊 395 个品规、33 条门诊主数据核验补丁、395 条门诊说明书资料覆盖（129 条严格匹配复用 + 266 条逐条补充）、`FX5882` 的 24片/盒确认状态、懒加载失败重试、双药库归属及跨库 ID 唯一性、分库笔记、全字段编辑、12 组高风险相互作用规则、临床四字段、商品名别名映射与来源、来源 URL 与核验日期、44 个作用分类，以及不得重新引入 OCR、相机或图片上传入口。
