...
const SEARCH_LINK_LIMIT = 16;
const FETCH_LINK_LIMIT = 12;
...
function sourceKind(hostname) {
  ...
  if (host === "39.net" || host.endsWith(".39.net") || host === "dxy.cn" || host.endsWith(".dxy.cn")) return "医药数据库";
  return "网页说明书来源";
}
...
function sourceScore(url) {
  ...
  if (kind === "官方/监管来源") return 110;
  if (kind === "药品说明书数据库") return 100;
  if (kind === "医药数据库") return 85;
  if (/manual|label|drug|medicine|medication|instruction|sm|shuomingshu|yaopin|product/i.test(parsed.pathname)) return 45;
  return 20;
}
...
async function browserSearch(query, env) {
  ...
  // 仍是全网搜索，但加入常见中文说明书数据库关键词，提升真实说明书页面的排序概率。
  const weightedQuery = `${query} 药品说明书 药源网 39药品通 丁香园 适应症 用法用量 不良反应 注意事项`;
  const searchUrl = `https://www.bing.com/search?q=${encodeURIComponent(weightedQuery)}&setlang=zh-Hans`;
...
async function findInstructionCandidates(query, env) {
  ...
  const cacheKey = new Request(`https://primary-medication-cache.invalid/instruction-v3?q=${encodeURIComponent(query)}`);
  if (cache) {
  ...
  const discoveredSourceHosts = [...new Set(searchLinks.map(item => item.host))].slice(0, 8);
  const result = { candidates, searchResultCount: searchLinks.length, fetchedSourceCount: linksToFetch.length, discoveredSourceHosts };
