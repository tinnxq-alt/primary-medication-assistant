...
- 当前只用于草稿验证；已创建独立的远程测试 D1 并部署独立 staging Worker，但尚未配置 Access，也未改变两个稳定站点。
...
- Worker 地址：`https://clinical-assistant-user-sync-staging.tinnxq.workers.dev`
- 初始版本：`cd69209d-ddc8-4617-a8a9-e9f672e8f74b`
- 未携带 Access 身份访问 `/health` 返回 `403 ACCESS_REQUIRED`，失败关闭已远程验证。
...
真正上线前还要完成：Cloudflare Zero Trust 免费计划初始化、Access 允许名单、数据保留/删除规则、备份恢复演练、前端首次迁移确认和临床机构的数据合规审批。
