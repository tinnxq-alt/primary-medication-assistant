...
assert.ok(Number(serviceWorker.match(/primary-medication-v(\d+)/)?.[1] || 0) >= 40, "安全修复后 PWA 缓存必须至少 v40");
...
assert.match(workerSource, /quickAction\("scrape"/, "Worker 应通过 Browser Run 打开真实搜索结果页");
assert.match(workerSource, /bing\.com\/search/, "当前全网说明书发现使用搜索引擎结果页");
assert.match(workerSource, /fetchSourcePage/, "搜索到候选后必须读取实际来源网页");
...
assert.match(workerSource, /mode:\s*"web-instruction-source-extraction"/, "Worker 必须标记为联网说明书原文抽取模式");
assert.match(workerSource, /generatesClinicalKnowledge:\s*false/, "健康检查必须明确不生成临床知识");
assert.doesNotMatch(workerSource, /env\.AI\.run|generateCandidateDetail|selected-candidate-detail/, "Worker 不得再让语言模型凭药名生成临床资料");
...
console.log("添加药物：联网检索真实说明书、来源原文抽取与自动填充检查通过");
